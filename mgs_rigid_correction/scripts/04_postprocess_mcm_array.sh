#!/bin/bash
# SLURM array script for postprocessing MCM ODB -> CSV extraction on the cluster.
# Each array task processes one ODB file from the flat input directory.

#SBATCH --job-name=mgs_mcm_postproc
#SBATCH --ntasks=1
...
METADATA_CSV="${FLAT_DIR}/run_metadata_full_mohr_coulomb.csv"
