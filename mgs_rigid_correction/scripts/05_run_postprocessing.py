...
def main():
    ...
    postprocessor_script = resolve_project_path(args.postprocessor_script)
    odb_path = resolve_project_path(args.odb_path) if args.odb_path else ""
    rows = selected_rows(read_csv(metadata_path), args.run_id, args.limit, args.require_odb)
    if not rows:
    ...
    for row in rows:
        ...
        if args.execute:
            ...
            rc = run_abaqus(args.abaqus_cmd, os.path.abspath(postprocessor_script), config_path)
            if rc != 0:
